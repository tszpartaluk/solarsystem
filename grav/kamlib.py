...
##################################################################################################################
#klasyczny oscylator harmoniczny, wzd³u¿ wzglêdem osi x

class SHO(field_custom):
	def __init__(self, y = 0.0, k = 1.0, b = 1.0):
		self.yaxis = y
		self.k = k
		self.b = b
	def force(self, obj):
		
		F = -(self.k*(obj.y - self.yaxis) + self.b*(obj.vy))
...
class SHO2(field_custom):
	...
	def force(self, obj):
		...
		F = -(self.k*(obj.y - self.yaxis) + self.b*(fabs(obj.vy)*obj.vy))
